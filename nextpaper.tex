...
The goal of this study is to estimate the \emph{causal effect} of code verifiability on code comprehensibility under controlled experimental conditions. We conduct a controlled experiment in which participants perform input-output comprehension tasks on semantically equivalent versions of Java methods that differ only in their verifiability with respect to the Checker Framework. Verifiability is operationalized relative to a fixed verification ecosystem, and is manipulated through semantics-preserving code transformations that induce or eliminate false positive warnings produced by selected checkers.

The study uses a \textbf{mixed design}: it is within subjects with respect to snippets---each participant completes tasks on all snippets---and between subjects with respect to verifiability condition---each participant sees only one version of each snippet, so no participant is exposed to both the high- and low-verifiability versions of the same underlying method. This design avoids learning and carryover effects that would arise if participants saw both versions of a snippet, while retaining the statistical efficiency of repeated measures across multiple snippets. Latin square counterbalancing ensures that across participants, both versions of each snippet are observed in balanced proportions. The analysis is conducted using linear mixed-effects models that account for variability across participants and snippets while estimating the fixed effect of verifiability condition.
...
\todo{These criteria seem too strict to me. I think it might be a good idea to permit a small amount of ``custom classes'', especially if we plan to remove Specimin to remove everything but their definitions. This means it's okay to 1) have instance methods, and 2) for there to be non-JDK classes referenced. That will greatly expand our pool of methods.}

\paragraph{Program Slicing}
For each candidate method, we construct a minimized executable program using \emph{Specimin}\todo{spelling is wrong: the tool is called ``Specimin''}, a program slicing tool. The resulting program contains only the code necessary to preserve the behavior relevant to the target method.\todo{This description of Specimin could be about any slicing tool. Be more precise.}\todo{This section should cite the Specimin paper.}
...
\todo{Do not use the ``subparagraph'' heading. Use subsubsection and paragraph instead; this text is not in a subsubsection.}
...
\paragraph{Semantics Preservation}
...
Variation in inherent difficulty, code structure, and familiarity is controlled through a paired design in which each underlying method appears in both high- and low-verifiability conditions across participants. This ensures that comparisons are made between semantically equivalent versions of the same code, holding essential complexity constant by construction. Critically, we do not rely on automatic complexity metrics (such as cyclomatic complexity, Halstead measures, or cognitive complexity) to establish equivalence between versions. Prior work has shown that such syntactic proxies are weak or unreliable predictors of actual comprehension difficulty~\cite{FeldmanKC2023}, and including them as covariates would not adequately control for the semantic dimensions of complexity that matter most for human understanding. Instead, equivalence of essential complexity is guaranteed structurally: because both versions implement the same algorithm and compute the same function, the inherent difficulty of the underlying problem is identical. Any residual accidental complexity introduced by the transformations is an acknowledged limitation, mitigated by restricting transformations to simple, well-understood patterns. Residual structural variation (e.g., code length) is measured and included as a covariate to account for surface-level differences.

To ensure that code length differences do not act as a systematic confound, transformations are selected to span a range of length changes: some transformations add lines, some remove lines, and some are length-neutral. This deliberate variation ensures that verifiability condition and code length are not collinear in the dataset, allowing the mixed-effects model to estimate each factor's contribution independently. We verify the absence of harmful collinearity using variance inflation factors (VIF) prior to interpreting model coefficients.
...
The study aims to isolate the causal effect of verifiability by using semantically equivalent code snippets that differ only in whether the checker produces false positive warnings. However, semantics-preserving transformations may introduce unintended structural differences---such as minor changes in control flow or code length---that could independently influence comprehension. We mitigate this risk by restricting transformations to simple, well-understood patterns and by manually validating input--output equivalence for each pair.

A related threat concerns complexity confounding. Because all automatic complexity proxies (cyclomatic complexity, Halstead measures, nesting depth, cognitive complexity) are known to be weak predictors of comprehension difficulty~\cite{FeldmanKC2023}, we cannot rely on them to verify that paired versions are complexity-equivalent. We address this structurally rather than statistically: since both versions implement the same algorithm, essential complexity is equal by construction. Residual accidental complexity differences introduced by the transformations are an acknowledged limitation; however, since transformations are minimal and well-understood, we argue that any observed effect on comprehension time is more plausibly attributed to verifiability than to incidental structural changes. As a secondary validation, a subset of snippet pairs is evaluated by experienced developers blind to the hypothesis, who judge whether the versions feel meaningfully different in complexity independent of the warning.

Code length differences across pairs are controlled by deliberately including transformations that add lines, remove lines, and are length-neutral. This prevents collinearity between verifiability condition and code length. Residual length differences are measured and included as a covariate, and VIF analysis is used to confirm that collinearity does not threaten coefficient interpretability.
